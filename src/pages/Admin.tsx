...
export default function Admin() {
  ...
  const [reportFilter, setReportFilter] = useState<'all' | 'purchased' | 'not_purchased' | 'interested'>('all');
  ...
  const filteredReport = participationReport.filter((item) => {
    ...
    if (reportFilter === 'interested') return item.was_interested;
    return true;
  });

  // Calculate interest stats
  const interestedCount = participationReport.filter(r => r.was_interested).length;
  const interestedAndPurchased = participationReport.filter(r => r.was_interested && r.purchased).length;
  const interestConversionRate = interestedCount > 0 ? Math.round((interestedAndPurchased / interestedCount) * 100) : 0;
              ...
              <div className="bg-card border border-border p-4 sm:p-6">
                ...
                <div className="flex flex-col gap-4 mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      onClick={() => setReportFilter('interested')}
                      className={`px-3 py-2 min-h-[44px] text-sm flex-1 sm:flex-none ${reportFilter === 'interested' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/80'}`}
                    >
                      <Heart className="w-4 h-4 inline mr-1" />
                      {language === 'nl' ? 'Geïnteresseerd' : 'Interested'}
                    </button>
                  </div>
                </div>
                ...
                {selectedDropForReport && participationReport.length > 0 && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-6">
                    ...
                    <div className="bg-primary/10 border border-primary/30 p-3 sm:p-4 text-center">
                      <p className="text-xl sm:text-2xl font-serif text-primary">{interestedCount}</p>
                      <p className="text-xs sm:text-sm text-muted-foreground">
                        <Heart className="w-3 h-3 inline mr-1" />
                        {language === 'nl' ? 'Interesse' : 'Interested'}
                      </p>
                      {interestedCount > 0 && (
                        <p className="text-xs text-muted-foreground mt-1">
                          → {interestedAndPurchased} ✓ ({interestConversionRate}%)
                        </p>
                      )}
                    </div>
                  </div>
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-border">
                            ...
                            <th className="text-left py-3 px-4 font-sans text-sm font-medium">
                              <Heart className="w-4 h-4 inline mr-1" />
                              {language === 'nl' ? 'Interesse' : 'Interest'}
                            </th>
                            <th className="text-left py-3 px-4 font-sans text-sm font-medium">{t.admin.memberStrikes}</th>
                            <th className="text-left py-3 px-4 font-sans text-sm font-medium">{t.admin.memberNotes}</th>
                          </tr>
                        ...
                        <tbody>
                          {filteredReport.map((item) => (
                            <tr key={item.member_id} className="border-b border-border/50">
                              ...
                              <td className="py-3 px-4">
                                {item.was_interested ? (
                                  <span className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-primary/10 text-primary">
                                    <Heart className="w-3 h-3 fill-current" />
                                    {item.interest_date && new Date(item.interest_date).toLocaleDateString()}
                                  </span>
                                ) : (
                        ...
                        <div key={item.member_id} className="bg-muted/30 border border-border p-4 space-y-2">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0">
                              {item.was_interested && (
                                <span className="inline-flex items-center px-2 py-1 text-xs bg-primary/10 text-primary">
                                  <Heart className="w-3 h-3 fill-current" />
                                </span>
                              )}
                              <span className={`px-2 py-1 text-xs ${item.purchased ? 'bg-secondary text-secondary-foreground' : 'bg-amber-500/20 text-amber-700'}`}>
                          ...
                          <div className="flex items-center gap-4 text-sm">
                            ...
                            {item.was_interested && item.interest_date && (
                              <span className="text-xs text-muted-foreground">
                                {language === 'nl' ? 'Interesse:' : 'Interest:'} {new Date(item.interest_date).toLocaleDateString()}
                              </span>
                            )}
                          </div>
                          {item.notes && (
